...
def _getinstalledDb2Path():
    ...
    if status == 0:
        ...
        _printOnly(
            "Please make sure you have set IBM_DB_HOME = "
            + path
        ...
        if _includeFlag:
            _printOnly(
                "**Found DB2 ODBC Driver from PATH, so skipping the clidriver download.**"
            )
...
def _checkGcc():
    ...
    gccFound = (
        (status == 0) or os.path.exists("/bin/gcc") or os.path.exists("/usr/bin/gcc")
    )
    ...
    class PostInstall(install):
        ...
        def run(self):
            ...
            for so in glob.glob(get_python_lib() + r"/ibm_db*.so"):
                os.system(
                    "install_name_tool -change @loader_path/clidriver/lib/libdb2.dylib {}/lib/libdb2.dylib {}".format(
                        clipath, so
                    )
                )
    ...
    class PostBuildExt(build_ext):
        ...
        def run(self):
            ...
            for so in glob.glob(self.build_lib + r"/ibm_db*.so"):
                os.system(
                    "install_name_tool -change libdb2.dylib {}/lib/libdb2.dylib {}".format(
                        clipath, so
                    )
                )
...
def _setWinEnv(name, value):
    ...
    if name not in old:
        ...
        envVal = (
            envVal
            + """os.environ['%(name)s'] = os.environ['%(name)s'] + ";" + os.path.join(ibm_db_home, 'bin')"""
            % {"name": name}
        )
        pyFile.write(envVal + "\n" + old)
    pyFile.close()
...
def _downloadClidriver(url):
    ...
    if os_ == "win":
        if sys.version_info[0:2] <= (2, 5):
            sys.stdout.write(
                "Auto installation of clidriver for Python Version %i.%i on Window platform is currently not supported \n"
...
def print_exception(e, url):
    if os_ == "win":
        err = (
            "Error while downloading clidriver from the following URL : "
            + url
            + "\n"
            + str(e)
        )
...
if "win32" in sys.platform:
    prebuildPYDname = ""
    if is64Bit:
        prebuildPYDname = "ibm_db64_py%i%i.pyd" % (
            sys.version_info[0],
            sys.version_info[1],
        )
    else:
        prebuildPYDname = "ibm_db32_py%i%i.pyd" % (
            sys.version_info[0],
            sys.version_info[1],
        )
...
if (ibm_db_home == "") and (ibm_db_dir == "") and (ibm_db_lib == ""):
    if "win32" not in sys.platform:
        sys.stdout.write(
            "Detected platform = " + sys.platform + ", uname = " + os.uname()[4] + "\n"
        )
    else:
        sys.stdout.write("Detected platform = " + sys.platform + "\n")
    if "zos" == sys.platform:
        _printAndExit(
            "You must set the environment variable IBM_DB_HOME to the HLQ of a DB2 installation."
        )
    ...
    easy_cli_path = os.path.join(
        tmp_path,
        "ibm_db-%s.egg"
        % (
            "-".join([VERSION, "py" + sys.version.split(" ")[0][0:3]])
            if ("win32" in sys.platform)
            else "-".join([VERSION, "py" + sys.version.split(" ")[0][0:3], os_, arch_])
        ),
        "clidriver",
    )
    ...
    if not os.path.isdir("clidriver"):
        ...
        try:
            ...
            try:
                git_url = (
                    "https://github.com/ibmdb/db2drivers/raw/main/clidriver/"
                    + cliFileName
                )
...
if not os.path.isdir(ibm_db_lib) and "zos" != sys.platform:
    ...
    if not os.path.isdir(ibm_db_lib):
        sys.stdout.write(
            "Cannot find %s directory. Check if you have set the IBM_DB_HOME environment variable's value correctly\n "
            % (ibm_db_lib)
        )
        sys.exit()
    notifyString = (
        "Detected usage of IBM Data Server Driver package. Ensure you have downloaded "
    )
    if is64Bit:
    ...
    notifyString = (
        notifyString
        + "of IBM_Data_Server_Driver and retry the ibm_db module install\n "
    )
...
if not prebuildIbmdbPYD and not os.path.isdir(ibm_db_include) and "zos" != sys.platform:
    sys.stdout.write(
        " %s/include folder not found. Check if you have set the IBM_DB_HOME environment variable's value correctly\n "
        % (ibm_db_dir)
    )
    sys.exit()
    ...
    if not os.path.isfile(library_x):
        ...
        if "DB2_MACS" not in os.environ:
            subprocess.run(
                [
                    "tso",
                    "oput '{}.SDSNMACS(DSNAO64C)' '{}'".format(
                        ibm_db_home, os.path.join(os.getcwd(), library_x)
                    ),
                ]
            )
        else:
            ...
            subprocess.run(
                [
                    "tso",
                    "oput '{}(DSNAO64C)' '{}'".format(
                        sdsn_macs, os.path.join(os.getcwd(), library_x)
                    ),
                ]
            )
...
package_data = {
    "ibm_db_tests": ["run_individual_tests", "*.png", "*.jpg", "config.py.sample"]
}

data_files = [
    (get_python_lib(), ["./README.md"]),
...
if "zos" == sys.platform:
    ext_modules = _ext_modules(
        os.path.join(os.getcwd(), include_dir), library, ibm_db_lib, ibm_db_lib_runtime
    )

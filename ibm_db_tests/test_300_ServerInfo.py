...
class IbmDbTestCase(unittest.TestCase):
    ...
    def run_test_300(self):
        ...
        if server:
            print(
                'DBMS_NAME: string(%d) "%s"' % (len(server.DBMS_NAME), server.DBMS_NAME)
            )
            ...
            print(
                'INST_NAME: string(%d) "%s"' % (len(server.INST_NAME), server.INST_NAME)
            )
            print(
                'SPECIAL_CHARS: string(%d) "%s"'
                % (len(server.SPECIAL_CHARS), server.SPECIAL_CHARS)
            )
            print("KEYWORDS: int(%d)" % len(server.KEYWORDS))
            print(
                'DFT_ISOLATION: string(%d) "%s"'
                % (len(server.DFT_ISOLATION), server.DFT_ISOLATION)
            )
            il = ""
            ...
            print(
                ...
                % (len(server.SQL_CONFORMANCE), server.SQL_CONFORMANCE)
            )
            print("PROCEDURES:", server.PROCEDURES)
            print(
                'IDENTIFIER_QUOTE_CHAR: string(%d) "%s"'
                % (len(server.IDENTIFIER_QUOTE_CHAR), server.IDENTIFIER_QUOTE_CHAR)
            )

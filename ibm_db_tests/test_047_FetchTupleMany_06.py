    ...
    def run_test_047(self):
        ...
        if server.DBMS_NAME[0:3] == "IDS":
            result = ibm_db.exec_immediate(
                conn, "SELECT empno, photo_format, photo_format from emp_photo"
            )
        else:
            result = ibm_db.exec_immediate(
                conn, "SELECT empno, photo_format, length(PICTURE) from emp_photo"
            )
        row = ibm_db.fetch_tuple(result)
        while row:
            if row[1] == "gif":
                print(
                    "<img src='test_047.php?EMPNO=%s&FORMAT=%s'><br>\n"
                    % (row[0], row[1])
                )
            if row[1] != "xwd":
                print(
                    "<a href='test_047.php?EMPNO=%s&FORMAT=%s' target=_blank>%s - %s - %s bytes</a>\n<br>"

...
class IbmDbTestCase(unittest.TestCase):
    ...
    def run_test_024(self):
        ...
        if conn != 0:
            ...
            statement = (
                "CREATE TABLE test_primary_keys (id INTEGER NOT NULL, PRIMARY KEY(id))"
            )
            result = ibm_db.exec_immediate(conn, statement)
            statement = "INSERT INTO test_primary_keys VALUES (1)"
            result = ibm_db.exec_immediate(conn, statement)
            statement = (
                "CREATE TABLE test_keys (name VARCHAR(30) NOT NULL, idf INTEGER NOT NULL, FOREIGN KEY(idf) REFERENCES test_primary_keys(id), \
                   PRIMARY KEY(name))"
            )
            result = ibm_db.exec_immediate(conn, statement)
            ...
            if server.DBMS_NAME[0:3] == "IDS":
                stmt = ibm_db.foreign_keys(
                    conn, None, None, None, None, config.user, "test_keys"
                )
            elif "zos" in sys.platform:
                stmt = ibm_db.foreign_keys(
                    conn, None, None, None, None, config.user, "TEST_KEYS"
                ...
                stmt = ibm_db.foreign_keys(
                    conn, None, None, None, None, None, "TEST_KEYS"
                )
            row = ibm_db.fetch_tuple(stmt)
            print(row[2])
            print(row[3])
            ...
            if server.DBMS_NAME[0:3] == "IDS":
                stmt = ibm_db.foreign_keys(
                    conn, None, config.user, "test_keys", None, None, None
                )
            elif "zos" in sys.platform:
                stmt = ibm_db.foreign_keys(
                    conn, None, config.user, "TEST_KEYS", None, None, None
                ...
                stmt = ibm_db.foreign_keys(
                    conn, None, None, "TEST_KEYS", None, None, None
                )
            row = ibm_db.fetch_tuple(stmt)
            print(row[2])
            print(row[3])
            ...
            if server.DBMS_NAME[0:3] == "IDS":
                ...
                stmt = ibm_db.foreign_keys(
                    conn, None, None, "TEST_KEYS", None, None, "TEST_FOREIGN_KEYS"
                )
            row = ibm_db.fetch_tuple(stmt)
            print(row[2])
            print(row[3])
            ...
            if server.DBMS_NAME[0:3] == "IDS":
                stmt = ibm_db.foreign_keys(
                    conn, None, config.user, "test_keys", None, "dummy_schema"
                )
            elif "zos" in sys.platform:
                stmt = ibm_db.foreign_keys(
                    conn, None, config.user, "TEST_KEYS", None, "dummy_schema"
                ...
                stmt = ibm_db.foreign_keys(
                    conn, None, None, "TEST_KEYS", None, "dummy_schema"
                )
            row = ibm_db.fetch_tuple(stmt)
            if not row:
                print("No Data Found")

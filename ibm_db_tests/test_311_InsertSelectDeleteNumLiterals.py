...
class IbmDbTestCase(unittest.TestCase):
    ...
    def run_test_311(self):
        ...
        if conn:
            ...
            stmt = ibm_db.prepare(
                conn, "SELECT col1, col2, col3 FROM tab_num_literals WHERE col1 = '11'"
            )
            ibm_db.execute(stmt)
            ...
            stmt = ibm_db.prepare(
                conn, "SELECT col1, col2, col3 FROM tab_num_literals WHERE col2 > '33'"
            )
            ibm_db.execute(stmt)
            data = ibm_db.fetch_both(stmt)
            while data:
            ...
            stmt = ibm_db.prepare(
                conn, "SELECT col1, col2, col3 FROM tab_num_literals WHERE col3 < '56'"
            )
            ibm_db.execute(stmt)
            data = ibm_db.fetch_both(stmt)
            while data:

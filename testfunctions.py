...
class IbmDbTestFunctions(unittest.TestCase):
    ...
    def testCasesIn(self, fileName):
        if (
            fileName.startswith("ibm_db_tests/test_017")
            or fileName.startswith("ibm_db_tests/test_005")
            or fileName.startswith("ibm_db_tests/test_018")
            or fileName.startswith("ibm_db_tests/test_019")
            or fileName.startswith("ibm_db_tests/test_024")
            or fileName.startswith("ibm_db_tests/test_053")
            or fileName.startswith("ibm_db_tests/test_054")
            or fileName.startswith("ibm_db_tests/test_080")
            or fileName.startswith("ibm_db_tests/test_081")
            or fileName.startswith("ibm_db_tests/test_082")
            or fileName.startswith("ibm_db_tests/test_090")
            or fileName.startswith("ibm_db_tests/test_091")
            or fileName.startswith("ibm_db_tests/test_092")
            or fileName.startswith("ibm_db_tests/test_103")
            or fileName.startswith("ibm_db_tests/test_116")
            or fileName.startswith("ibm_db_tests/test_133")
            or fileName.startswith("ibm_db_tests/test_147")
            or fileName.startswith("ibm_db_tests/test_157a")
            or fileName.startswith("ibm_db_tests/test_240")
            or fileName.startswith("ibm_db_tests/test_241")
            or fileName.startswith("ibm_db_tests/test_cursortype")
            or fileName.startswith("ibm_db_tests/test_decfloat")
            or fileName.startswith("ibm_db_tests/test_setgetOption")
            or fileName.startswith("ibm_db_tests/test_warn")
        ):
            return True
        else:
            return False
    ...
    def expected_LUW(self, fileName):
        ...
        fileInput = (
            fileHandle.read()
            .split("#__LUW_EXPECTED__")[-1]
            .split("#__ZOS_EXPECTED__")[0]
            .replace("\n", "")
            .replace("#", "")
        )
        fileHandle.close()
    ...
    def expected_IDS(self, fileName):
        ...
        fileInput = (
            fileHandle.read()
            .split("#__IDS_EXPECTED__")[-1]
            .split("#__ZOS_ODBC_EXPECTED__")[0]
            .replace("\n", "")
            .replace("#", "")
        )
        fileHandle.close()
    ...
    def expected_ZOS(self, fileName):
        ...
        fileInput = (
            fileHandle.read()
            .split("#__ZOS_EXPECTED__")[-1]
            .split("#__SYSTEMI_EXPECTED__")[0]
            .replace("\n", "")
            .replace("#", "")
        )
        fileHandle.close()
    ...
    def expected_AS(self, fileName):
        ...
        fileInput = (
            fileHandle.read()
            .split("#__SYSTEMI_EXPECTED__")[-1]
            .split("#__IDS_EXPECTED__")[0]
            .replace("\n", "")
            .replace("#", "")
        )
        fileHandle.close()
    ...
    def assert_expect(self, testFuncName):
        ...
        try:
            if self.server.DBMS_NAME[0:2] == "AS":
                self.assertEqual(
                    self.capture(testFuncName), self.expected_AS(callstack[1][1])
                )
            elif (
                platform.system() == "z/OS"
                or platform.system() == "OS/390"
                and self.testCasesIn(callstack[1][1])
            ):
                self.assertEqual(
                    self.capture(testFuncName), self.expected_ZOS_ODBC(callstack[1][1])
                )
            elif self.server.DBMS_NAME == "DB2" or "DSN" in self.server.DBMS_NAME:
                self.assertEqual(
                    self.capture(testFuncName), self.expected_ZOS(callstack[1][1])
                )
            elif self.server.DBMS_NAME[0:3] == "IDS":
                self.assertEqual(
                    self.capture(testFuncName), self.expected_IDS(callstack[1][1])
                )
            else:
                self.assertEqual(
                    self.capture(testFuncName), self.expected_LUW(callstack[1][1])
                )
